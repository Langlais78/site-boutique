                ...
                <div className="space-y-4">
                    <div className="card-glow rounded-[24px] border border-white/10 bg-[var(--surface)] p-6">
                        ...
                        <p className="mt-3 text-2xl font-semibold text-[var(--ink)]">
                            {formatPrice(product.sale_price_cents ?? product.price_cents, product.currency)}
                        </p>
                        ...
                        <p className="mt-4 text-xs uppercase tracking-[0.22em] text-[var(--muted)]">
                            Stock {product.stock ?? 0} · {product.is_active ? 'Actif' : 'Inactif'}
                        </p>
                    </div>
                    ...
                    <div className="card-glow rounded-[24px] border border-white/10 bg-[var(--surface)] p-6">
                        ...
                        <div className="mt-4 grid gap-2 text-sm text-[var(--muted)]">
                            <p><span className="text-[var(--ink)]">SKU:</span> {product.sku ?? '—'}</p>
                            <p><span className="text-[var(--ink)]">Slug:</span> {product.slug}</p>
                            <p><span className="text-[var(--ink)]">Marque:</span> {product.brand ?? '—'}</p>
                            <p><span className="text-[var(--ink)]">Categorie:</span> {product.category ?? '—'}</p>
                            {product.weight_grams ? (
                                <p><span className="text-[var(--ink)]">Poids:</span> {product.weight_grams} g</p>
                            ) : null}
                            {product.dimensions ? (
                                <p>
                                    <span className="text-[var(--ink)]">Dimensions:</span>{' '}
