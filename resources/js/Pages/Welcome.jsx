...
export default function Welcome({ featuredProducts = [] }) {
    ...
    const formatPriceCents = (cents, currency) =>
        new Intl.NumberFormat("fr-FR", {
            style: "currency",
    ...
    return (
        <AuthenticatedLayout>
                    ...
                    <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-stretch">
                        <div className="space-y-7">
                            <h1 className="font-['Chakra_Petch'] text-4xl font-semibold leading-tight text-[var(--ink)] sm:text-5xl lg:text-6xl">
                                Des bornes d'arcade personnalisees, fabriquees a
                                la main et pretes a jouer.
                            </h1>
                            <p className="max-w-xl text-base text-[var(--muted)]">
                                Barbu Studio cree des cabinets uniques et
                                propose une boutique d'accessoires impression
                                3D, gaming et tech pour booster vos setups.
                            </p>
                        ...
                        <div className="neon-border relative h-full overflow-hidden rounded-[28px] bg-[var(--surface)] p-3">
                            <div className="relative h-full min-h-[340px] overflow-hidden rounded-[22px] border border-white/10 bg-[linear-gradient(135deg,rgba(19,24,38,0.9),rgba(9,12,20,0.9))]">
                                <img
                                    src="/images/borne.png"
                                    alt="Borne arcade personnalisee"
                                    className="absolute inset-0 h-full w-full origin-top object-cover object-[96%_8%] scale-[1.08] -translate-y-[4%]"
                                />
                                <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(8,10,18,0),rgba(8,10,18,0.75))]" />
                                <div className="absolute top-4 left-4 rounded-full border border-white/15 bg-[rgba(8,10,18,0.75)] px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.25em] text-[#23f0ff]">
                                    Borne signature
                                </div>
                            </div>
                        </div>
            ...
            <div className="mx-auto w-full max-w-6xl px-6">
                <div className="my-12 h-px bg-[linear-gradient(90deg,transparent,rgba(255,204,128,0.7),transparent)]" />
            </div>
            <section className="relative mx-auto w-full max-w-6xl px-6 pb-20">
                <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
                    <div className="card-glow rounded-[28px] border border-white/10 bg-[var(--surface)] p-8">
                        <p className="text-xs font-semibold uppercase tracking-[0.35em] text-[var(--muted)]">
                            L'atelier Barbu Studio
                        </p>
                        <h2 className="mt-3 font-['Chakra_Petch'] text-3xl font-semibold text-[var(--ink)]">
                            Artisan passionne, finitions soignees.
                        </h2>
                        <p className="mt-4 text-sm text-[var(--muted)]">
                            Chaque piece est testee, calibree et ajustee a la
                            main. Le studio melange impression 3D, electronique
                            et design pour des experiences arcade uniques.
                        </p>
                        <div className="mt-6 grid gap-4 sm:grid-cols-2">
                            {[
                                "Cabinets sur mesure et habillages premium",
                                "Pieces 3D imprimees pour modders et makers",
                                "Accessoires gaming selectionnes a la main",
                                "Support direct et SAV humain",
                            ].map((item) => (
                                <div
                                    key={item}
                    ...
                    <div className="grid gap-4">
                        {[
                            {
                                title: "Print 3D & accessoires",
                                text: "Pieces robustes, finitions propres, pr�tes a monter.",
                            },
                            {
                                title: "Borne arcade personnalisable",
                                text: "Config box, ecran, joystick, skin et console.",
                            },
                            {
                                title: "Accessoires gaming",
                                text: "Peripheriques, audio et upgrades setup.",
                            },
                        ].map((step, index) => (
                            <div
                                key={step.title}
                            ...
                            >
                                <p className="text-[10px] font-semibold uppercase tracking-[0.25em] text-[#23f0ff]">
                                    Focus {index + 1}
                                </p>
                ...
                <div className="grid gap-6 lg:grid-cols-3">
                    {[
                        ...
                        },
                        {
                            title: "Borne arcade personnalisable",
                            description:
                                "Configurations uniques, finitions premium, livraison cl� en main.",
                        ...
                        >
                            ...
                            <Link
                                href={route("boutique")}
                                className="mt-6 inline-flex rounded-full border border-white/15 px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.25em] text-[var(--ink)] transition hover:border-[#23f0ff] hover:text-[#23f0ff]"
                            >
                                Explorer
                            </Link>
                        </div>
