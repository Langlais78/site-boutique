...
const formatOptions = (items?: { id: number; name: string }[]) =>
    items ?? [];

type AccessoriesByType = Record<string, { id: number; name: string }[]>;

type Props = {
    accessories?: AccessoriesByType;
};

export default function CustomArcade({ accessories = {} }: Props) {
    ...
    const lockBox = Boolean(screenId || joystickId || consoleId || skinId || targetId);
                ...
                <div className="card-glow rounded-[28px] border border-white/10 bg-[var(--surface)] p-6">
                    ...
                    <div className="mt-6 grid gap-4">
                        <div>
                            ...
                            >
                                <option value="" disabled={lockBox}>Choisir</option>
                        ...
                        {boxId && (
                            <div>
                                ...
                                <select
                                    ...
                                    className="mt-2 h-12 w-full rounded-2xl border border-white/10 bg-[var(--surface-2)] px-4 text-sm text-[var(--ink)]"
                                >
                                    <option value="" disabled={lockScreen}>Choisir</option>
                                    {formatOptions(accessories.screen).map((item) => (
                                        <option key={item.id} value={item.id}>
                                            {item.name}
                                        </option>
                                    ))}
                        ...
                        {screenId && (
                            <div>
                                ...
                                <select
                                    ...
                                    className="mt-2 h-12 w-full rounded-2xl border border-white/10 bg-[var(--surface-2)] px-4 text-sm text-[var(--ink)]"
                                >
                                    <option value="" disabled={lockJoystick}>Choisir</option>
                                    {formatOptions(accessories.joystick).map((item) => (
                                        <option key={item.id} value={item.id}>
                                            {item.name}
                                        </option>
                                    ))}
                        ...
                        {joystickId && (
                            <div>
                                ...
                                <select
                                    ...
                                    className="mt-2 h-12 w-full rounded-2xl border border-white/10 bg-[var(--surface-2)] px-4 text-sm text-[var(--ink)]"
                                >
                                    <option value="" disabled={lockConsole}>Choisir</option>
                                    {formatOptions(accessories.console).map((item) => (
                                        <option key={item.id} value={item.id}>
                                            {item.name}
                                        </option>
                                    ))}
                        ...
                        {consoleId && (
                            <div>
                                ...
                                <select
                                    ...
                                    className="mt-2 h-12 w-full rounded-2xl border border-white/10 bg-[var(--surface-2)] px-4 text-sm text-[var(--ink)]"
                                >
                                    <option value="" disabled={lockSkin}>Choisir</option>
                                    {formatOptions(accessories.skin).map((item) => (
                                        <option key={item.id} value={item.id}>
                                            {item.name}
                                        </option>
                                    ))}
                        ...
                        {skinId && (
                            <div>
                                ...
                                >
                                    ...
                                    {formatOptions(accessories.target).map((item) => (
                                        <option key={item.id} value={item.id}>
                                            {item.name}
                                        </option>
                                    ))}
